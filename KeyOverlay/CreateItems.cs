...
namespace KeyOverlay
{
    ...
    {
        private static readonly Font Font = new Font(Path.GetFullPath(Path.Combine
        ...
        public static RectangleShape CreateBar(RectangleShape square, int outlineThickness, float barSpeed)
        {
            ...
            rect.Position = new(square.Position.X - outlineThickness,
        ...
        public static Text CreateText(string key, RectangleShape square, Color color, bool counter)
        {
            ...
            text.Origin = new(text.GetLocalBounds().Width / 2f, 32 * square.Size.X / 140f);
            if (counter)
            {
                text.Position = new(square.GetGlobalBounds().Left + square.OutlineThickness + square.Size.X / 2f,
        ...
        {
            var bytes = s.Split(',').Select(byte.Parse).ToArray();
